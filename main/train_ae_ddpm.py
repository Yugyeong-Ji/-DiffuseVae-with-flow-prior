...
def train(config):
    ...
    ddpm_wrapper = DDPMWrapper(
        ...
        vae_weight=config.training.vae_weight,  # VAE 손실 가중치
        ddpm_weight=config.training.ddpm_weight,  # DDPM 손실 가중치
        anneal_start=config.training.anneal_start,  # Annealing 시작 에포크
        anneal_end=config.training.anneal_end,  # Annealing 종료 에포크
    )
