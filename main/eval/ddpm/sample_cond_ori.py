...
@hydra.main(config_path="/home/dxlab/jupyter/hanyong/DiffuseVAE/main/configs", config_name="test_ori.yaml")
def sample_cond(config):
    ...
    vae = VAE.load_from_checkpoint(
        ...
        strict=False  # VAE 로드시 strict=False
    )
    vae.eval()
    ...
    decoder = UNetModel(
        in_channels=3,  # 6에서 3으로 변경
        model_channels=128,  # 160에서 128로 변경 (체크포인트와 일치)
        out_channels=3,
        num_res_blocks=config_ddpm.model.n_residual,
        attention_resolutions=attn_resolutions,
        channel_mult=(1,2,2,2),  # dim_mults 고정
    ...
    # DDPM 체크포인트를 로드하고 VAE 관련 키 제거
    checkpoint = torch.load(config_ddpm.evaluation.chkpt_path)
    state_dict = checkpoint['state_dict']
    state_dict = {k: v for k, v in state_dict.items() if not k.startswith('vae.')}
