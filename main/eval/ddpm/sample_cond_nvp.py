...
from collections import OrderedDict  # 키 리매핑에 필요
...
def remap_checkpoint_keys(ckpt, prefix_to_remove="vae.flow_prior.", new_prefix="flow_prior."):
    """
    체크포인트 키의 접두사를 변경하는 함수.
    기존 키의 prefix를 제거하고 새 prefix로 교체합니다.
    """
    new_state_dict = OrderedDict()
    for k, v in ckpt.items():
        if k.startswith(prefix_to_remove):
            new_key = k.replace(prefix_to_remove, new_prefix, 1)
...
def sample_cond(config):
    ...
    # Load pretrained VAE 체크포인트 불러오기
    vae_ckpt = torch.load(config_vae.evaluation.chkpt_path, map_location="cpu")

    # Flow Prior 키 리매핑
    vae_ckpt['state_dict'] = remap_checkpoint_keys(vae_ckpt['state_dict'])
    print("Flow Prior 키 리매핑 완료")
