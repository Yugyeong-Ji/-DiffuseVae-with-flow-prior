...
class VAE(pl.LightningModule):
    ...
    def set_prior(self, flow_prior):
        self.flow_prior = flow_prior    

    def encode(self, x):
        mu, logvar = self.enc(x)
        if self.flow_prior is not None:
            # 입력 데이터의 채널을 확장
            target_channels = 512  # RealNVP에서 기대하는 채널 수
            if mu.shape[1] != target_channels:
                mu = mu.repeat(1, target_channels // mu.shape[1], 1, 1)
            z, log_det_jacobian = self.flow_prior(mu)
            return z, logvar + log_det_jacobian
        return mu, logvar
    ...
    def on_save_checkpoint(self, checkpoint):
        # Flow Prior의 상태 저장
        if self.flow_prior is not None:
            checkpoint["flow_prior_state"] = self.flow_prior.state_dict()

    def on_load_checkpoint(self, checkpoint):
        # Flow Prior의 상태 로드 (Optional - 오류 방지)
        if self.flow_prior is not None and "flow_prior_state" in checkpoint:
            try:
                self.flow_prior.load_state_dict(checkpoint["flow_prior_state"], strict=False)
            except RuntimeError as e:
                print(f"Warning: Flow Prior load failed - {e}")
