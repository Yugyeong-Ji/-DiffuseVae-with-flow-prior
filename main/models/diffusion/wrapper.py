...
class DDPMWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # VAE 손실 계산
        recons_loss = nn.MSELoss()(cond, x) 
        kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / x.size(0)  # KL 손실
        vae_loss = recons_loss + 0.01 * kl_loss 
        ...
        anneal_factor = min(1.0, max(0.0, (current_epoch - self.anneal_start) / (self.anneal_end - self.anneal_start)))

        # 가중치 조정된 총 손실 계산
        total_loss = (1 - anneal_factor) * self.vae_weight * vae_loss + anneal_factor * self.ddpm_weight * ddpm_loss

        # 그래디언트 클리핑 및 최적화
        optim.zero_grad()
        self.manual_backward(total_loss)
        torch.nn.utils.clip_grad_norm_(
            self.online_network.parameters(), self.grad_clip_val
        )
        torch.nn.utils.clip_grad_norm_(
        ...
        self.log("anneal_factor", anneal_factor, prog_bar=True)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            [
                {"params": self.online_network.parameters()},  # DDPM 전체를 학습
                {"params": self.vae.parameters()},             # VAE 전체를 학습
            ],
            lr=self.lr,
        )
